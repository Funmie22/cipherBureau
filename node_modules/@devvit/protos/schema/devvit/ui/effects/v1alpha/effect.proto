...
enum EffectType {
  ...
  EFFECT_REALTIME_SUB = 0 [deprecated = true];

  // Trigger a re-render for apps that have visible UI
  EFFECT_RERENDER_UI = 1 [deprecated = true];
  ...
  EFFECT_SET_INTERVALS = 7 [deprecated = true];

  // This starts a purchase flow
  EFFECT_CREATE_ORDER = 8 [deprecated = true];

  // Control and communicate with WebViews
  EFFECT_WEB_VIEW = 9 [deprecated = true];

  // Check if the app can execute some actions as the user
  EFFECT_CAN_RUN_AS_USER = 11 [deprecated = true];

  // Statistical data event for analytics.
  EFFECT_TELEMETRY = 12 [deprecated = true];

  // Request an updated signed request context
  EFFECT_UPDATE_REQUEST_CONTEXT = 13 [deprecated = true];

  // Send the screenshot response from web view to the client
  EFFECT_SCREENSHOT_RESPONSE = 14 [deprecated = true];

  // Request a login/signup prompt
  EFFECT_LOGIN_PROMPT = 15 [deprecated = true];
}

// Devvit Classic effects. Most are deprecated. Don't add Webbit effects here.
message Effect {
  oneof effect_type {
    RealtimeSubscriptionsEffect realtime_subscriptions = 1 [deprecated = true];
    RerenderEffect rerender_ui = 2 [deprecated = true];
    ...
    SetIntervalsEffect interval = 9 [deprecated = true];
    devvit.ui.effect_types.v1alpha.CreateOrderEffect create_order = 10 [deprecated = true];
    devvit.ui.effects.web_view.v1alpha.WebViewEffect web_view = 11 [deprecated = true];
    devvit.ui.effect_types.v1alpha.CanRunAsUserEffect can_run_as_user = 13 [deprecated = true];
  }
